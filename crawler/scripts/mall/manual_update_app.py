...
class ManualUpdateApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("엑셀 배송정보 수동 업데이트")
    ...
    def _detect_repo_root(self) -> Path:
        ...
        for candidate in candidates:
            ...
            if (candidate / "package.json").exists() and (
                candidate / "scripts" / "mall" / "fill_sheet2_delivery.py"
            ).exists():
                return candidate
    ...
    def _build_ui(self) -> None:
        ...
        title = tk.Label(frame, text="원할 때 실행: 엑셀 파일 업데이트", font=(_font, 16, "bold"))
    ...
    def _pick_python_cmd(self) -> str:
        ...
        # frozen(.app) 실행 시 sys.executable은 앱 바이너리이므로
        # Python 인터프리터 후보에서 제외하지 않으면 앱이 재실행될 수 있다.
    ...
    def _pick_worker_exe(self) -> str:
        ...
        env_worker = os.environ.get("MALL_FILL_WORKER_BIN", "").strip()
        if env_worker:
        ...
        candidates.extend(
            [
                exe_path.with_name("FillSheetWorker.exe"),
                exe_path.parent / "FillSheetWorker.exe",
                self.repo_root / "runtime" / "FillSheetWorker.exe",
                self.repo_root / "app" / "FillSheetWorker.exe",
            ]
        )
    ...
    def _choose_output(self) -> None:
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=Path(initial_path).name if initial_path else "updated.xlsx",
            filetypes=[("Excel 파일", "*.xlsx"), ("모든 파일", "*.*")],
        )
        if file_path:
    ...
    def _poll_logs(self) -> None:
        while True:
            ...
            if kind == "log":
                ...
                self._append_log("[DONE] 엑셀 업데이트 완료")
                self._set_running(False)
                messagebox.showinfo("완료", f"업데이트 파일 생성 완료\n{payload}")
            elif kind == "error":
